...
class LoadHugeImageFromFile(LoadImageFromFile):
    def __init__(self,
        ...
        super().__init__(to_float32, color_type, imdecode_backend, file_client_args, ignore_empty)
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = super().transform(results)['img']
        h, w = img.shape[:2]
        windows = self._slide_window(
            width=w,
            height=h,
            sizes=[self.patch_size],
            steps=[self.patch_step],
        )
        # windows = self._slide_window2(
        #     width=w,
        #     height=h,
        #     size=self.patch_size,
        # )
        self.patch_num = len(windows)
        
        results['img_path'] = results['img_path']
        results['img'] = img
        results['windows'] = windows
        results['patch_shape'] = (self.patch_size, self.patch_size)
        results['patch_step'] = self.patch_step
        results['patch_batch_size'] = self.patch_bs
        results['patch_num'] = self.patch_num
        results['img_shape'] = img.shape[:2]
        results['ori_shape'] = img.shape[:2]
        return results
    
    def _slide_window(self, width, height, sizes, steps):
        """Generate a non-overlapping sliding window grid."""
        assert len(sizes) == len(steps), '`sizes` and `steps` must align'
        windows = []
        for size, step in zip(sizes, steps):
            assert step >= size
            for y in range(0, height, size):
                y1 = y
                y2 = min(y + size, height)
    ...
    def _slide_window2(self, width: int, height: int, size: int) -> np.ndarray:
        """Generate grid windows by adjusting strides to avoid tiny edges."""

        def _gen_positions(length: int, win_size: int) -> List[int]:
            if length <= win_size:
                return [0]
            num = int(np.ceil(length / win_size))
            if num <= 1:
                return [0]
            positions = np.linspace(0, length - win_size, num, dtype=np.int64)
            return positions.tolist()

        x_positions = _gen_positions(width, size)
        y_positions = _gen_positions(height, size)
